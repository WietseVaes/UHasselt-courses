...
clear xc
clear yc
clear Selem Melem Cxelem Cyelem
...
for index1 = 1:topology
	for index2 = 1:topology
		Selem(index1,index2) = abs(Delta)*permeability/viscosity*(beta(index1)*beta(index2)+gamma(index1)*gamma(index2));
    end
end

Melem = Delta/24*(ones(3)+eye(3));
Cxelem = -permeability*abs(Delta)/(6*viscosity)*[beta; beta; beta];
Cyelem = -permeability*abs(Delta)/(6*viscosity)*[gamma; gamma; gamma];